...
async def main(config_path: str):
    ...
    # Pre-normalize symbol_overrides (HA UI passes string)
    so = raw.get("symbol_overrides", "")
    if isinstance(so, str):
        so = so.strip()
        raw["symbol_overrides"] = (yaml.safe_load(so) if so else {}) or {}
    ...
    tg = Telegram(token=(cfg.tg_token or "").strip() or None,
                  chat_id=(cfg.tg_chat_id or "").strip() or None).fallback_from_env()
    ...
    # Warmup with 300 bars (как было в 1.6.0)
