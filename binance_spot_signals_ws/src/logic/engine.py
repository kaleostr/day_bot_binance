...
class SignalEngine:
    """Логика сигналов: LONG-only, grouped 3/5 (trend+momentum+liquidity)"""
    ...
    def evaluate(self, sym: str)->str|None:
        ...
        # VWAP location: must be above or reclaim with volume
        ...
        trend_ok = cond_bias_ema_1h or cond_ema200_15m_above or cond_ema200_15m_reclaim

        # Momentum (at least one)
        ...
        momentum_ok = (cond_macd_up and cond_rsi_reclaim50) if require_macd_and_rsi else (cond_macd_up or cond_rsi_reclaim50)

        # Liquidity (at least one)
        ...
        liquidity_ok = cond_vwap_above or cond_volume_spike

        # Count 3 of 5: [trend_any, macd_up, rsi_reclaim50, vwap_above, volume_spike]
        checks = [trend_ok, cond_macd_up, cond_rsi_reclaim50, cond_vwap_above, cond_volume_spike]
        ok_cnt = sum(bool(x) for x in checks)
        if not (trend_ok and momentum_ok and liquidity_ok and ok_cnt >= confirmations_min):
            return None

        # --- Entry/Stops/Targets ---
        entry = max(h[-1], c[-1]) * 1.0001  # tiny tunnel
        R = atr[-1] * atr_sl_mult
        sl = entry - R
        tp1 = entry + self.tp_multipliers[0] * R
        tp2 = entry + self.tp_multipliers[1] * R
        tp3 = entry + self.tp_multipliers[2] * R

        trail_atr = c[-1] - self.atr_trailing_mult * atr[-1]
        trail_vwap = vwap_val if np.isfinite(vwap_val) else trail_atr
        trail = max(trail_atr, trail_vwap) if self.use_vwap_trailing else trail_atr
