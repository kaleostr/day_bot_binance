...
async def on_startup():
    ...
    try:
        opts = get_addon_options()
        tg = TelegramNotifier(opts.get("telegram_token",""), opts.get("telegram_chat_id",""))
        ver = json.load(open("/config.json","r"))["version"] if False else None
    except Exception:
        tg = None
    try:
        cfg = json.load(open(str(Path(__file__).resolve().parents[1] / "config.json"), "r", encoding="utf-8"))
        version = cfg.get("version","")
    except Exception:
        version = ""
    if tg:
        await tg.send(f"✅ Binance Spot Signal Bot запущен{f' (v{version})' if version else ""}")
